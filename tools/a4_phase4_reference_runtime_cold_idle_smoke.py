...
ARTIFACT_ROOT = ROOT / "artifacts" / "a4_phase4_reference_runtime_cold_idle"
FIXTURE_ROOT = ARTIFACT_ROOT / "project"
SCRIPT = "res://tests/a4_phase4_reference_runtime_cold_idle_smoke.gd"
MARKER = "WT_TERRAIN_A4_PHASE4_GODOT_PASS"
ENGINE_VERSIONS = ("4.7",)
...
def prepare_fixture() -> None:
    ...
    copy_file(
        ROOT / "tests" / "a4_phase4_reference_runtime_cold_idle_smoke.gd",
        FIXTURE_ROOT / "tests" / "a4_phase4_reference_runtime_cold_idle_smoke.gd",
    )
    (FIXTURE_ROOT / "project.godot").write_text(
        "\n".join(
            [
                ...
                "",
                "[application]",
                'config/name="World Transvoxel Terrain A4 Phase 4 Fixture"',
...
def run_import(version: str, engine: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"A4 phase 4 fixture import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A4 phase 4 runtime/cold-idle smoke failed on {version}")
    marker_line = next(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A4 phase 4 runtime/cold-idle smoke harness."
    )
    ...
    report_path = ARTIFACT_ROOT / "a4_phase4_reference_runtime_cold_idle_report.json"
