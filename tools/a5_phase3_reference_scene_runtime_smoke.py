...
ARTIFACT_ROOT = ROOT / "artifacts" / "a5_phase3_reference_scene_runtime"
...
def prepare_fixture() -> None:
    ...
    copy_file(
        ROOT / "tests" / "a5_phase3_reference_scene_runtime_smoke.gd",
        FIXTURE_ROOT / "tests" / "a5_phase3_reference_scene_runtime_smoke.gd",
    )
    (FIXTURE_ROOT / "project.godot").write_text(
        "\n".join(
            [
                ...
                "",
                "[application]",
                'config/name="World Transvoxel Terrain A5 Phase 3 Fixture"',
...
def run_import(version: str, engine: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"A5 phase 3 fixture import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A5 phase 3 reference scene runtime smoke failed on {version}")
    marker_line = next(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A5 phase 3 reference scene runtime smoke harness."
    )
    ...
    report_path = ARTIFACT_ROOT / "a5_phase3_reference_scene_runtime_report.json"
