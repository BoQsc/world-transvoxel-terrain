...
ARTIFACT_ROOT = ROOT / "artifacts" / "a2_addon_smoke"
SCRIPT = "res://tests/a2_addon_smoke.gd"
MARKER = "WT_TERRAIN_A2_GODOT_SMOKE_PASS"
ENGINE_VERSIONS = ("4.7",)
...
def discover_engines(explicit: list[Path]) -> list[tuple[str, Path]]:
    ...
    raise RuntimeError(
        "No Godot executable found. Pass --godot or set GODOT."
    )
...
def run_import(version: str, engine: Path) -> None:
    ...
    if result.returncode != 0 or has_godot_error(combined):
        raise RuntimeError(f"Godot import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A2 addon smoke failed on {version}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A2 addon smoke harness."
    )
    ...
    ARTIFACT_ROOT.mkdir(parents=True, exist_ok=True)
    report_path = ARTIFACT_ROOT / "a2_addon_smoke_report.json"
    report_path.write_text(
        json.dumps({"engines": results}, indent=2) + "\n", encoding="utf-8"
    )
    print(
