...
def run_import(version: str, engine: Path) -> None:
    ...
    if result.returncode != 0 or has_godot_error(combined):
        raise RuntimeError(f"A4 phase 1 import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A4 phase 1 resource smoke failed on {version}")
    marker_line = next(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A4 phase 1 resource smoke harness."
    )
    ...
    report_path = ARTIFACT_ROOT / "a4_phase1_resources_report.json"
