...
ARTIFACT_ROOT = ROOT / "artifacts" / "a5_phase4_debug_overlay_categories"
...
def prepare_fixture() -> None:
    ...
    copy_file(
        ROOT / "tests" / "a5_phase4_debug_overlay_categories_smoke.gd",
        FIXTURE_ROOT / "tests" / "a5_phase4_debug_overlay_categories_smoke.gd",
    )
    (FIXTURE_ROOT / "project.godot").write_text(
        "\n".join(
            [
                ...
                "",
                "[application]",
                'config/name="World Transvoxel Terrain A5 Phase 4 Fixture"',
...
def run_import(version: str, engine: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"A5 phase 4 fixture import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A5 phase 4 debug overlay smoke failed on {version}")
    marker_line = next(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A5 phase 4 debug overlay category smoke harness."
    )
    ...
    report_path = ARTIFACT_ROOT / "a5_phase4_debug_overlay_categories_report.json"
