...
ARTIFACT_ROOT = ROOT / "artifacts" / "a3_bridge_smoke"
...
def prepare_fixture() -> None:
    ...
    copy_file(ROOT / "tests" / "a3_bridge_smoke.gd", FIXTURE_ROOT / "tests" / "a3_bridge_smoke.gd")
...
def run_import(version: str, engine: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError(f"A3 fixture import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A3 bridge smoke failed on {version}")
    marker_line = next(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A3 bridge smoke harness."
    )
    ...
    report_path = ARTIFACT_ROOT / "a3_bridge_smoke_report.json"
    report_path.write_text(
        json.dumps(
            {
                "fixture_root": str(FIXTURE_ROOT.relative_to(ROOT).as_posix()),
                "dependency_source": str(WORLD_TRANSVOXEL_ADDON),
                "engines": results,
            },
            indent=2,
        )
