...
ARTIFACT_ROOT = ROOT / "artifacts" / "a5_phase1_debug_snapshot"
SCRIPT = "res://tests/a5_phase1_debug_snapshot_smoke.gd"
MARKER = "WT_TERRAIN_A5_PHASE1_GODOT_PASS"
ENGINE_VERSIONS = ("4.7",)
...
def run_import(version: str, engine: Path) -> None:
    ...
    if result.returncode != 0 or has_godot_error(combined):
        raise RuntimeError(f"A5 phase 1 import failed on {version}")
...
def run_smoke(version: str, engine: Path) -> dict[str, object]:
    ...
    if result.returncode != 0 or MARKER not in combined or has_godot_error(combined):
        raise RuntimeError(f"A5 phase 1 debug snapshot smoke failed on {version}")
    marker_line = next(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the world-transvoxel-terrain A5 phase 1 debug snapshot smoke harness."
    )
    ...
    ARTIFACT_ROOT.mkdir(parents=True, exist_ok=True)
    report_path = ARTIFACT_ROOT / "a5_phase1_debug_snapshot_report.json"
