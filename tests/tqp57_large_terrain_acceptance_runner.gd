...
func _run() -> void:
	...
	capture_root = OS.get_environment("WT_ACCEPTANCE_CAPTURE_ROOT") if OS.has_environment("WT_ACCEPTANCE_CAPTURE_ROOT") else CAPTURE_ROOT
...
func _run_scenario(
	...
	for frame in range(frame_count):
		...
		render_cpu_values.append(RenderingServer.viewport_get_measured_render_time_cpu(get_tree().root.get_viewport_rid()) * 1000.0)
		render_gpu_values.append(RenderingServer.viewport_get_measured_render_time_gpu(get_tree().root.get_viewport_rid()) * 1000.0)
